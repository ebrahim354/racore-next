...
class ViewInventory extends React.Component {
  ...
  saveItem = async index => {
    ...
    try{
      const res = await axios.put(`${url + productsRoute}/${inventory[index].id}`, {"updates": inventory[index]}, config);
  ...
  deleteItem = async index => {
    ...
    try{
      const res = await axios.delete(`${url + productsRoute}/${this.state.inventory[index].id}`, config);
      console.log('del', res);
    } catch (err){
      console.log(err);
    }
